...
When in Two-Button Joystick mode, the adapter supports different button mappings, better suited to different game genres. The mappings can be switched by pressing <kbd>Select</kbd> in combination with other buttons. LD1 will blink quickly a few times to indicate what mapping has been activated.
